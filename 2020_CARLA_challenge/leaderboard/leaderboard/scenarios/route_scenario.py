...
class RouteScenario(BasicScenario):
    ...
    def _draw_waypoints(self, world, waypoints, vertical_shift, persistency=-1):
        ...
        for w in waypoints:
            ...
            world.debug.draw_point(
                wp, size=size, color=color, life_time=persistency)
    ...
    def _scenario_sampling(self, potential_scenarios_definitions, random_seed=0):
        ...
        def select_scenario(list_scenarios):
            ...
            for scenario in list_scenarios:
                try:
                    scenario_number = int(
                        scenario['name'].split('Scenario')[1])
                except:
                    scenario_number = -1
        ...
        for trigger in potential_scenarios_definitions.keys():
            ...
            while position_sampled(scenario_choice, sampled_scenarios):
                ...
                del possible_scenarios[possible_scenarios.index(
                    scenario_choice)]
    ...
    def _get_actors_instances(self, list_of_antagonist_actors):
        ...
        if 'front' in list_of_antagonist_actors:
            list_of_actors += get_actors_from_list(
                list_of_antagonist_actors['front'])

        if 'left' in list_of_antagonist_actors:
            list_of_actors += get_actors_from_list(
                list_of_antagonist_actors['left'])

        if 'right' in list_of_antagonist_actors:
            list_of_actors += get_actors_from_list(
                list_of_antagonist_actors['right'])
